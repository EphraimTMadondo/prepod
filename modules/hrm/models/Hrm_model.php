...
class hrm_model extends App_Model
{
    ...
    public function get_job_position($id = false)
    {
        ...
        if ($id == false) {
            ...
            return $this->db->query("select * from tbljob_position WHERE company_username = '$companyusername'")->result_array();
        }
    ...
    public function get_job_position_arrayid()
    {
        ...
        $position = $this->db->query("select * from tbljob_position WHERE company_username = '$companyusername'")->result_array();
        $position_arrray = [];
    ...
    {
        $companyusername = $_SESSION['current_company'];
        return $this->db->query("select * from tblworkplace WHERE company_username = '$companyusername'")->result_array();
    }
    ...
    public function get_contracttype(){
        $companyusername = $_SESSION['current_company'];
        // $this->db->where('tblstaff_contracttype.company_username', $companyusername);
        return $this->db->query("select * from tblstaff_contracttype WHERE company_username ='$companyusername'")->result_array();
    }
    ...
    public function get_salary_form($id = false){
        ...
        if ($id == false) {
            $companyusername = $_SESSION['current_company'];
        return $this->db->query("select * from tblsalary_form WHERE company_username = '$companyusername' ")->result_array();
        }
    ...
    public function get_contract($id){
        ...
        $page = $this->db->query('select * from tblstaff_contract where id_contract = '.$id)->result_array();
        if ($page['company_username'] ==  $companyusername)
        {
                        return $page;
        }
      ...
      public function get_contractOverdue(){
         ...
         $this->db->where('end_valid < ',date('Y-m-d'));
         $this->db->where('company_username', $companyusername);
          

       return $this->db->get(db_prefix() . 'staff_contract')->result_array();

    }
    
     public function get_contractExpire(){
          
          $companyusername = $_SESSION['current_company'];
          
         $this->db->where('end_valid <=',date('Y-m-d',strtotime('+7 day',strtotime(date('Y-m-d')))));
          $this->db->where(' end_valid >=', date('Y-m-d'));
         $this->db->where('company_username', $companyusername);
...
public function evalmath($equation)
{
   ...
   return $result;
    ...
    public function get_hrm_dashboard_data(){
        ...
        $new_staff_in_month = $this->staff_model->getThisMonth();
        ...
        $staff_working = $this->staff_model->getWorking();
        $staff_birthday = $this->staff_model->getBirthdays;
       ...
       $overdue_contract = $this->get_contractOverdue();
       // $overdue_contract = [];
        $expire_contract = $this->get_contractExpire();
