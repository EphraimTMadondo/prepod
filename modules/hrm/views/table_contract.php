...
$companyusername = $_SESSION['current_company'];
 array_push($where, 'AND ('.db_prefix()."staff_contract.company_username = '$companyusername')");
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'staff_contract.id_contract', 'name_contracttype', 'firstname','type_name', 'allowance_val', 'form_name', 'salary_val', 'duration', 'unit']);
...
foreach ($rResult as $aRow) {
    ...
    $row[] = ' <a href="' . admin_url('hrm/member/' . $aRow['staff']) . '">' . $aRow['firstname'] . '</a>';
