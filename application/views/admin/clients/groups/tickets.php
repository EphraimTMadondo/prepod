...
if(isset($client)){
    ...
    if(($total_active_contacts > 0)
        ...
        echo '<a href="'.($total_active_contacts > 1 ? '#' : admin_url('tickets/add?contact_id='.$contacts[0]['id'].'&userid='.$client->userid)).'" class="btn btn-secondary'.($total_active_contacts > 1 ? ' dropdown-toggle' : '').'"'.($total_active_contacts > 1 ? ' data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"' : '').'>';
        echo _l('new_ticket');
