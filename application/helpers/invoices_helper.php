...
function get_invoices_percent_by_status($status)
{
    ...
    $total_invoices      = total_rows(db_prefix() . 'invoices',db_prefix() . "invoices.company_username ='$companyusername'", 'status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));

    $data            = [];
    $total_by_status = 0;
    if (!is_numeric($status)) {
        if ($status == 'not_sent') {
                        $total_by_status = total_rows(db_prefix() . 'invoices','sent=0 AND status NOT IN(2,5)' ." AND ".db_prefix() . "invoices.company_username ='$companyusername'" . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));

        }
    } else {
        
                $total_by_status = total_rows(db_prefix() . 'invoices', 'status = ' . $status . ' AND status NOT IN(5)' ." AND ".db_prefix() . "invoices.company_username ='$companyusername'" .(!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
