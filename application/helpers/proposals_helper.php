...
function get_proposals_percent_by_status($status, $total_proposals = '')
{
    ...
    if (!is_numeric($total_proposals)) {
                        $total_proposals = total_rows(db_prefix() . 'proposals',"company_username ='$companyusername'", $whereUser);

    }

    $data            = [];
    $total_by_status = 0;
        $where           = 'status=' . get_instance()->db->escape_str($status)." AND company_username ='$companyusername'";
