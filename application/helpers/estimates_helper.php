...
function get_estimates_percent_by_status($status, $project_id = null)
{
    ...
    if (endsWith($where, ' AND')) {
        ...
        $total_estimates = total_rows(db_prefix().'estimates',"company_username ='$companyusername'",  $where);
    ...
    if (!is_numeric($status)) {
        if ($status == 'not_sent') {
        $total_by_status = total_rows(db_prefix().'estimates','sent=0 AND status NOT IN(2,3,4)' ." AND company_username ='$companyusername'". ($where != '' ? ' AND (' . $where . ')' : ''));

        }
    } else {
        $whereByStatus = 'status=' . $status." AND company_username ='$companyusername'";
        if ($where != '') {
