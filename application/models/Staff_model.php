...
class Staff_model extends App_Model
{
    ...
    public function getThisMonth($id = '', $where = [])
    {
        ...
        $this->db->select('staffid');
         $this->db->where('company_username', $companyusername);
          ...
          $this->db->where('MONTH(datecreated)', date('m'));
            ...
            //return $this->db->query('SELECT * FROM tblstaff WHERE MONTH(datecreated) = '.date('m').' AND YEAR(datecreated) = '.date('Y'))->result_array();

    return $this->db->get(db_prefix() . 'staff')->result_array();
    }
    public function getWorking($id = '', $where = [])
    {
    ...
    public function getBirthdays($id = '', $where = [])
    {
        ...
        $this->db->select('staffid');
         $this->db->where('company_username', $companyusername);
          ...
          $this->db->where('MONTH(birthday)', date('m'));
           $this->db->where('YEAR(birthday)', date('Y'));
...
public function test2()
{
     ...
     // $this->db->select('staffid');
            // $this->db->where('company_username', $companyusername);
         // return $this->db->get(db_prefix() . 'staff_companies')->result_array();
        /**
         $staffArray =[];
            $staffArray3 =[];
               foreach ($staffArray as $value){ 
