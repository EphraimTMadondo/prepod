...
class Spam_filters extends AdminController
{
    ...
    public function view($rel_type, $filter_type = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            array_push($where, 'AND ('.db_prefix()."spam_filters.company_username = '$companyusername')");
            $result       = data_tables_init($aColumns, $sIndexColumn, $sTable, [], [
                'AND type ="' . $filter_type . '" AND rel_type="' . $rel_type . '"'.'AND ('.db_prefix()."spam_filters.company_username = '$companyusername')",
            ], [
                'id',
            ]);
